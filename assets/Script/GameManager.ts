...
export class GameManager extends Component {
    ...
    private createSelfPlayer(roleData: RoleMessage): void {
        if (this.selfPrefab) {
            ...
            if (this.selfScript) {
                ...
                this.selfScript.action(roleData.getPositionX(), roleData.getPositionY(), roleData.getFaceAngle());
            } else {
    ...
    onBattleOperation(roles: RoleMessage[]): void {
        for (let i = 0; i < roles.length; i++) {
            if (roles[i].roleId !== this.roleId) {
                this.targetScript.action(roles[i].getPositionX(), roles[i].getPositionY(), roles[i].getFaceAngle());
            } else {
                this.selfScript.action(roles[i].getPositionX(), roles[i].getPositionY(), roles[i].getFaceAngle());
            }
        }
    }
